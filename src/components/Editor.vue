<template>
    ...
    <choose-image :limit="9" ref="ChooseImageRef" :preview="false"></choose-image>
</template>
<script setup>
...
const init = {
    ...
    setup: (editor) => {
        editor.ui.registry.addButton("imageUpload", {
            ...
            icon: "image",
            onAction() {
                // 当提交图片时候 open传递一个回调函数 通过回调函数拿到图片数据
                ChooseImageRef.value.open((data) => {
                    data.forEach((url) => {
                        editor.insertContent(`<img src=${url} style="widht:100%"  >`)
                    })
...
const editorTarget = tinymce.init; // 初始化
